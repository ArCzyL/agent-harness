...
1. **会话失忆与 Token 巨额消耗**：AI 编程助手跨会话没有任何持久记忆。每次新开窗口，它都要盲目地全局 `grep` 搜索几百个源文件，光是“搞清楚项目架构”这一步就要白白烧掉 **40 多万个 Token（约 \$1.50 ~ \$6.00）**。
...
* 🧠 **本地持久化知识图谱（Token 消耗降低 99%）**：
  基于纯 C 语言编写的高性能单二进制引擎（`codebase-memory-mcp`）。将代码 AST、调用链、接口路由与类型系统持久化存入本地 SQLite，**查询延迟低于 1 毫秒，将结构性探索的 Token 消耗从 41 万暴降至 3400 个**。
* 🥋 **Karpathy 防翻车四大行为准则（行为围栏）**：
  ...
  * **目标驱动执行 (Goal-Driven Execution)**：将每个任务变成可量化的测试指标；必须跑通测试才准交差。
* 🔒 **物理测试闭环（Test Harness Loop）**：
  自动嗅探项目技术栈（Go, Rust, TypeScript/Node, Python 等），自动绑定终端编译与测试命令。强制 AI 进入自动化自愈闭环：  
...
* 🌐 **100% 跨开发工具（IDE-Agnostic）**：
  一套配置，全网通用。自动适配 **TRAE、Cursor、Claude Code、Antigravity、Windsurf、Zed、VS Code**。以 `AGENTS.md` 为唯一真实源，并自动创建 `CLAUDE.md`、`GEMINI.md`、`.cursorrules` 等软链接兼容老工具。
* 🛡️ **私有仓库零泄露保护**：
...
2. 扫描本机已安装的 AI 工具（TRAE、Cursor、Claude Code、Antigravity 等）并自动注入全局 MCP 配置；
...
### 智能体闭环流程：
1. **防误覆盖保护**：若项目根目录下已有你精心编写的 `AGENTS.md`，**绝对跳过生成**，保护既有规则；
2. **AST 架构自省**：自动解析项目主要语言比例、模块目录与 main 入口点并存入知识图谱；
3. **定制化生成 `AGENTS.md`**：自动根据技术栈填入专属的测试命令（Harness）与 Karpathy 四大纪律；
4. **跨工具软链接同步**：自动生成 `CLAUDE.md -> AGENTS.md` 等软链接，确保无论用哪款工具打开都 100% 生效。
...
| :--- | :---: | :---: |
| **5 次结构性探索 Token** | ~412,000 Tokens | **~3,400 Tokens (降低 99.2%)** |
| **架构查询延迟** | 数秒至十几秒 | **< 1 毫秒** |
| **代码过度设计与架构漂移** | 频繁（随意添加复杂设计模式） | **被 Karpathy 准则 2 严格拦截** |
| **交付未测试的暗坑 Bug** | 家常便饭（空口凭感觉说修好了） | **被自动化测试 Harness 物理围栏锁死** |
| **多 IDE 规则割裂与重复维护** | 严重（各个编辑器规则格式互不通用） | **统一以 AGENTS.md 为准，自动兼容** |
...
| **Zed / OpenCode** | 标准配置路径 | 原生识别 `AGENTS.md` |
